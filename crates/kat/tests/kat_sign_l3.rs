//! KAT validation for SQIsign Level 3 signing.
//!
...
use sqisign_rs::{Level3, Verifier};

const KAT_RSP: &str = include_str!("../../../reference/KAT/PQCsignKAT_529_SQIsign_lvl3.rsp");
...
fn kat_sign_level3() {
    ...
    let precomp = SigningPrecomp::<Level3>::level3();
    ...
    entries.par_iter().for_each(|entry| {
        ...
        // 1. Keygen
        let (pk, sk) = protocols_keygen::<Level3>(&mut rng, &precomp);
        ...
        let mut sk = sqisign_rs::SecretKey::<Level3>::from_bytes(&sk_bytes)
            .expect("sk round-trip must succeed");
        sk.populate_from_pk(&pk);

        // 2. Sign
        let sig =
            protocols_sign::<Level3>(&pk, &sk, &entry.msg, &mut rng).expect("signing must succeed");
