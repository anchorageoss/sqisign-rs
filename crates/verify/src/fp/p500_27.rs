//! Field arithmetic modulo `p = 27 * 2^500 - 1` (505 bits).
//!
//! Origin: SQIsign round 3, NIST level III (also round 2, level V).
//!
//! Elements are 9 limbs of 57 bits (unsaturated, 513 bits of
//! storage) in Montgomery form with `R = 2^513`. Arithmetic results
//! are lazily reduced to `[0, 2p)`; encoding and comparison reduce
//! fully. `2^456` divides `p + 1`, so the Montgomery reduction folds
//! each emitted low limb `v` back in as `v * 27 * 2^44` at limb
//! position 8.
//!
//! GENERATED by `tools/gen-fp`. Do not edit; change the generator.
//!
//! On x86-64 the trait implementation lives in `p500_27_asm.rs` (saturated
...
/// `(p + 1) / 2^456`: the contribution of `p + 1` to the top limb.
/// Montgomery folding constant in `modmul` and `modsqr`.
pub const PTOP: u64 = 0x1b00000000000;

/// `2 * PTOP`, used to add or subtract `2p` in the lazy reductions.
pub const TWO_PTOP: u64 = 0x3600000000000;
...
/// Montgomery form of `2^504`: shifts the accumulator by one
/// 63-byte chunk in `fp_decode_reduce`.
...
/// Propagate carries so limbs `0..8` are in `[0, 2^RADIX)` and the top
...
pub(crate) fn flatten(n: &mut [u64; NLIMBS]) -> u32 {
    ...
    n[8] = n[8].wrapping_add(PTOP & carry);
    let _ = prop(n);
    (carry & 1) as u32
}
...
pub(crate) fn modadd(n: &mut [u64; NLIMBS], a: &[u64; NLIMBS], b: &[u64; NLIMBS]) {
    ...
    n[8] = n[8].wrapping_sub(TWO_PTOP);
    let carry = prop(n);
    n[0] = n[0].wrapping_sub(2 & carry);
    n[8] = n[8].wrapping_add(TWO_PTOP & carry);
    let _ = prop(n);
}
...
/// `c <- a * b * R^-1 mod 2p`. Schoolbook 9x9 product in radix `2^57`
/// with interleaved Montgomery folding: each emitted low limb `v_i` is
/// added back at position `i + 8` as `v_i * PTOP`, which is
...
/// (499 squarings, 101 multiplications after a 16-entry table).
