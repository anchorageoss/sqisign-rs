//! Field arithmetic modulo `p = 3 * 2^324 - 1` (326 bits).
//!
//! Origin: SQIsign round 3, NIST level I.
//!
//! Elements are 6 limbs of 55 bits (unsaturated, 330 bits of
//! storage) in Montgomery form with `R = 2^330`. Arithmetic results
//! are lazily reduced to `[0, 2p)`; encoding and comparison reduce
//! fully. `2^275` divides `p + 1`, so the Montgomery reduction folds
//! each emitted low limb `v` back in as `v * 3 * 2^49` at limb
//! position 5.
//!
//! GENERATED by `tools/gen-fp`. Do not edit; change the generator.
//!
//! On x86-64 the trait implementation lives in `p324_3_asm.rs` (saturated
...
pub const ENCODED_BYTES: usize = 41;

/// `(p + 1) / 2^275`: the contribution of `p + 1` to the top limb.
/// Montgomery folding constant in `modmul` and `modsqr`.
pub const PTOP: u64 = 0x6000000000000;

/// `2 * PTOP`, used to add or subtract `2p` in the lazy reductions.
pub const TWO_PTOP: u64 = 0xc000000000000;
...
/// Montgomery form of `2^320`: shifts the accumulator by one
/// 40-byte chunk in `fp_decode_reduce`.
...
/// Propagate carries so limbs `0..5` are in `[0, 2^RADIX)` and the top
...
pub(crate) fn prop(n: &mut [u64; NLIMBS]) -> u64 {
    ...
    carry >>= RADIX;
    n[0] &= MASK;
    for limb in n.iter_mut().take(5).skip(1) {
        carry = carry.wrapping_add(*limb as i64);
        *limb = (carry as u64) & MASK;
        carry >>= RADIX;
    }
    n[5] = n[5].wrapping_add(carry as u64);
    0u64.wrapping_sub(n[5] >> 63)
}
...
pub(crate) fn flatten(n: &mut [u64; NLIMBS]) -> u32 {
    ...
    n[5] = n[5].wrapping_add(PTOP & carry);
    let _ = prop(n);
    (carry & 1) as u32
}
...
pub(crate) fn modadd(n: &mut [u64; NLIMBS], a: &[u64; NLIMBS], b: &[u64; NLIMBS]) {
    ...
    n[5] = n[5].wrapping_sub(TWO_PTOP);
    let carry = prop(n);
    n[0] = n[0].wrapping_sub(2 & carry);
    n[5] = n[5].wrapping_add(TWO_PTOP & carry);
    let _ = prop(n);
}
...
/// `c <- a * b * R^-1 mod 2p`. Schoolbook 6x6 product in radix `2^55`
/// with interleaved Montgomery folding: each emitted low limb `v_i` is
/// added back at position `i + 5` as `v_i * PTOP`, which is
...
/// (319 squarings, 65 multiplications after a 16-entry table).
