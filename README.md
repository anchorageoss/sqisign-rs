...
>
> **Not SUF-CMA.** SQIsign is not strongly unforgeable and cannot be made
> so: a valid signature has other byte strings the verifier accepts. This
> crate does not try to change that. See
> [ePrint 2026/1305](https://eprint.iacr.org/2026/1305).
...
549 bytes, in a `Zeroizing<Vec<u8>>`). The RustCrypto `signature` traits
are implemented (`Verifier`, `SignatureEncoding`, `RandomizedSigner`),
and a `PreparedPublicKey` (`pk.prepare()`) does the per-key work once for
a verifier that sees many signatures under one key. For a verifier only,
...
| x86-64 without them | the same saturated layout's portable kernels, 2.1 to 2.9x slower at verification |
...
build and 1.15 to 1.7x faster than its portable `ref` build; a prepared
